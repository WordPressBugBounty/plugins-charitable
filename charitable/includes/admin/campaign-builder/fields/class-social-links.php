...
if ( ! class_exists( 'Charitable_Field_Social_Links' ) ) :
	...
	class Charitable_Field_Social_Links extends Charitable_Builder_Field {
		...
		public function render( $field_data = false, $campaign_data = false, $field_id = false, $mode = 'template', $template_data = false ) {
			...
			if ( 'template' === $mode ) {
				$html .= '<div class="charitable-field-' . $mode . '-social-linking-headline-container">';
			} else {
				$html .= '<div class="charitable-field-' . $mode . '-social-linking-headline-container charitable-placeholder">';
			}

			$html .= '<h5 class="charitable-field-' . $mode . '-headline">' . trim( $headline ) . '</h5></div>
